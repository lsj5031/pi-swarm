...
CLEANUP=false         # Run cleanup after completion
MERGE_PRS=false       # Attempt to merge PRs
...
Options:
  ...
  --cleanup               Clean up orphaned issues, PRs, and worktrees
  --merge-prs             Attempt to merge open PRs after completion
...
Examples:
  ...
  commander.sh --resume --cleanup    # Clean up after a run
EOF
    exit 0
}
...
run_cleanup() {
    ...
    if [[ -d ".worktrees" ]]; then
        ...
        for dir in .worktrees/*/; do
            if [[ -d "$dir" ]]; then
                rm -rf "$dir" 2>/dev/null && stale_count=$((stale_count + 1))
            fi
        done
        [[ $stale_count -gt 0 ]] && info "Removed $stale_count stale worktrees"
    fi
    
    # Clean up stale branches
    log "Cleaning stale branches..."
    local branch_count=0
    for branch in $(git branch | grep -E 'issue/|epic/|review/' 2>/dev/null); do
        git branch -D "$branch" 2>/dev/null && branch_count=$((branch_count + 1))
    done
    [[ $branch_count -gt 0 ]] && info "Removed $branch_count stale branches"
    
    # Close orphaned issues (created but not in plan)
    if [[ -f "$PLAN_FILE" ]]; then
        log "Checking for orphaned issues..."
        local planned_epics
        planned_epics=$(jq -r '.epic_waves[].epics[]' "$PLAN_FILE" 2>/dev/null | sort -u)
        
        # Get issues created by commander (have "Generated by Commander" in body)
        local orphaned=0
        gh issue list --state open --limit 500 --json number,body | \
            jq -r '.[] | select(.body | contains("Generated by Commander")) | .number' | \
            while read num; do
                if ! echo "$planned_epics" | grep -q "^$num$"; then
                    gh issue close "$num" --comment "Closing: Orphaned issue not in execution plan." 2>/dev/null && orphaned=$((orphaned + 1))
                fi
            done
        [[ $orphaned -gt 0 ]] && info "Closed $orphaned orphaned issues"
    fi
    
    success "Cleanup complete"
}

# Attempt to merge open PRs
run_merge_prs() {
    log "🔀 Attempting to merge open PRs..."
    
    local prs merged=0 failed=0
    prs=$(gh pr list --state open --json number,title,mergeable | jq -r '.[] | select(.mergeable == "MERGEABLE") | .number')
    
    if [[ -z "$prs" ]]; then
        info "No mergeable PRs found"
        return 0
    fi
    ...
    for pr in $prs; do
        ...
        info "Merging PR #$pr: $title"
        
        if gh pr merge "$pr" --squash --delete-branch 2>/dev/null; then
            success "Merged PR #$pr"
            merged=$((merged + 1))
            
            # Close associated issue if PR title contains "Closes #X"
            local issue_num
            issue_num=$(echo "$title" | grep -oE '#[0-9]+' | head -1 | tr -d '#')
            if [[ -n "$issue_num" ]]; then
                gh issue close "$issue_num" --comment "Closed by PR #$pr" 2>/dev/null || true
            fi
        else
    ...
    info "Merged: $merged, Failed: $failed"
}
...
main() {
    ...
    # Run post-completion tasks
    if [[ "$MERGE_PRS" == true ]]; then
        echo ""
        run_merge_prs
    fi
