...
NO_MERGE=false        # Pass --no-merge to captains
...
Options:
  ...
  --cleanup               Clean up orphaned worktrees and branches
  --no-merge              Pass --no-merge to captains (don't merge PRs)
  -j, --jobs <n>          Max parallel jobs per captain wave
  -h, --help              Show this help

Note: PRs are merged by captain after each wave by default. Use --no-merge to disable.
...
execute_epic() {
    ...
    [[ "$NO_MERGE" == true ]] && captain_cmd+=" --no-merge"
...
# Cleanup orphaned resources (worktrees and branches only - captain handles PR merging)
run_cleanup() {
    log "🧹 Running cleanup..."
